...
open(post_kernel_file, ">${post_kernelfilename}"); # extracted kernel goes here
open(kernel_file, ">${kernelfilename}"); # everything after kernel goes here 
...
foreach $line (@lines) {
  ...
  # if we haven't encountered the kernel yet, chekc for __global__
  if (!$foundKernel) {
    ($word) = (m/^\s*(\w+)/x);
    $foundKernel = $word eq __global__;
  }
