...
template<class TData> void runTest(int packedElementSize){
    ...
    cutilSafeCall( cudaMemcpy(h_odataGPU, d_odata, MEM_SIZE, cudaMemcpyDeviceToHost) );
    int flag = testCPU(
        (TData *)h_odataGPU,
        (TData *)h_idataCPU,
        numElements,
...
int main(int argc, char **argv){
    ...
    printf("Uploading input data to GPU memory...\n");
        cutilSafeCall(cudaMemcpy(d_idata, h_idataCPU, MEM_SIZE, cudaMemcpyHostToDevice) );
