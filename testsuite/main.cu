...
{   
    ...
    cutilSafeCall( cudaMalloc( (void**) &(input.g_a), 
                              sizeof(float) * mat_size));
    cutilSafeCall( cudaMalloc( (void**) &(input.g_b_raw),
                              sizeof(float) * mat_size));

    // copy data to device
    cutilSafeCall( cudaMemcpy( input.g_a, input.a, sizeof(float) * mat_size,
                              cudaMemcpyHostToDevice ));
    cutilSafeCall( cudaMemcpy( input.g_b_raw, input.b, 
                              sizeof(float) * mat_size,
                              cudaMemcpyHostToDevice ));
